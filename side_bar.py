...
def sidebar_render():
    with st.sidebar:
        ...
        #                 new_url = upload_json_card(model_card, repo_id, token)
        ...
        #     missing_required = validation_utils.validate_required_fields(SCHEMA, st.session_state, current_task=task)
        #     # We'll store the latest README text in session_state
        #     if "last_readme_text" not in st.session_state:
        #         st.session_state.last_readme_text = None

        #     with st.form("Generate README.md from current card"):
        ...
        #             st.text_area("README.md", value=st.session_state.last_readme_text, height=300)


        #     st.markdown("## Export README.md to Hub")
        #     with st.form("Upload README to Hub"):
        #         st.markdown("Use a token with write access from [here](https://hf.co/settings/tokens)")
        #         token_rm = st.text_input("Token", type="password")
        #         repo_id_rm = st.text_input("Repo ID (e.g. user/repo)")
        #         push_rm = st.form_submit_button("Upload README.md")

        #     if push_rm:
        #         if len(repo_id_rm.split('/')) == 2:
        #             try:
        #                 # Reusa el estado para generar README
        #                 card_content = parse_into_json(SCHEMA)
        #                 card_obj = json.loads(card_content) if isinstance(card_content, str) else card_content
        #                 readme_text = build_readme_from_card(card_obj)

        #                 # Tu card_upload ya escribe "README.md"; si pasas un string lo respeta
        ...
        # Put this near the top, before st.tabs(...)
        st.markdown("""
        <style>
