...
def appendix_render():
    ...
    # Check for deletions from top upload list
    deleted_files = []
    ...
    if st.session_state.appendix_uploads:
        ...
        for original_name, file_data in st.session_state.appendix_uploads.items():
            ...
            # Preview directly if supported
                        # Preview in expander only if supported
            file_path = st.session_state.appendix_uploads[original_name]["path"]
            file_ext = Path(file_path).suffix.lower()

            try:
                preview_supported = False
                if file_ext in [".png", ".jpg", ".jpeg", ".gif", ".pdf", ".txt", ".csv", ".json", ".md", ".py", ".c", ".cpp", ".h"]:
