...
def appendix_render():
    ...
    # Ensure the shared registry for the renderer exists
    ...
    if uploaded_files:
        for file in uploaded_files:
            if file.name not in st.session_state.appendix_uploads:
                ...
                # Store basic appendix metadata
                st.session_state.appendix_uploads[file.name] = {
                    "custom_label": "",
                    "path": save_path,
                }

                # 🔗 Register for md_renderer normalization (_normalize_file_from_key)
                key = f"appendix_{file.name}"
                st.session_state.render_uploads[key] = {
                    "path": save_path,
                    "name": file.name,
                }
    ...
    # Compute deletions (files that were removed from the uploader UI)
    deleted_files = []
    current_file_names = [f.name for f in uploaded_files] if uploaded_files else []
    ...
    # Apply deletions and clean up both local files and registries
    for file_to_delete in deleted_files:
        try:
            os.remove(st.session_state.appendix_uploads[file_to_delete]["path"])
        except Exception:
        ...
        # Remove from render_uploads so the renderer won't reference stale files
        try:
            key = f"appendix_{file_to_delete}"
            st.session_state.render_uploads.pop(key, None)
        except Exception:
            pass

        # Remove appendix metadata
        del st.session_state.appendix_uploads[file_to_delete]
